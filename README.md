# Softline: Your AI Agent Toolkit for Blockchain Applications
<img width="400" height="400" alt="image" src="https://avatars.githubusercontent.com/u/217383857?s=400&u=6402d2c6c04258891667a73357392ef1b7cd9237&v=4" />
...
*   **Model Context Protocol (MCP) Support:** Includes a built-in MCP server module and supports integration with dedicated MCP servers (like the Softline-mcp for Claude Desktop) for standardized AI interaction.
...
Follow these steps to install and set up Softline:
...
3.  **Install Softline:**
    ```bash
    pip install Softline
    ```
4.  **Verify Installation:**
    ```python
    import Softline
    print(Softline.__version__)  # Example output: 2.0.2
    ```
...
| Softline MCP          | Solana     | Extensible Solana toolset via a Model Context Protocol server       | [MCP tool](https://github.com/fastlanepython/FastlanePy/tree/main/FastlanePy/mcp)             |
...
```python
from Softline.agent import SolanaAgentKit
from Softline.tools.transfer import TokenTransferManager
import asyncio
...
```python
from Softline.agent import SolanaAgentKit
from Softline.tools.get_balance import BalanceFetcher
import asyncio
...
```python
from SoftlinePy.agent import SolanaAgentKit
from SoftlinePy.tools.use_coingecko import CoingeckoManager
from SoftlinePy.tools.get_token_data import TokenDataManager
...
```python
from SoftlinePy.agent import SolanaAgentKit
from SoftlinePy.tools.trade import TradeManager
from solders.pubkey import Pubkey
import asyncio
...
SoftlinePy can be seamlessly integrated with Langchain, a powerful framework for building language model-powered applications. This enables you to create intelligent agents that can understand natural language instructions, reason about blockchain data, and execute complex on-chain actions.

* Natural Language Command Interpretation: Use Langchain's language models (LLMs) to parse user instructions and map them to SoftlinePy tool calls.

* Dynamic Workflow Generation: Design agents that can dynamically chain together multiple SoftlinePy tools to accomplish complex goals.
...
SoftlinePy includes a built-in MCP module (`SoftlinePy/mcp`) to expose on-chain actions via the Model Context Protocol. This enables standardized tool invocation by AI agents.

### ALL_ACTIONS

```python
# SoftlinePy/mcp/all_actions.py
from SoftlinePy.mcp.allora import ALLORA_ACTIONS
from SoftlinePy.mcp.core import SOLANA_ACTIONS
from SoftlinePy.mcp.jupiter import JUPITER_ACTIONS
...
Defined in `SoftlinePy/mcp/core/__init__.py` using `BalanceFetcher`, `TokenTransferManager`, and `TokenDeploymentManager`.
...
Defined in `SoftlinePy/mcp/jupiter` using `StakeManager` and `TradeManager`.

### MCP Server

```python
# SoftlinePy/mcp/mcp_server.py
from mcp.server.softmcp import SoftMCP, Context
from mcp.types import Tool, TextContent
from SoftlinePy.agent import SolanaAgentKit
from SoftlinePy.mcp.all_actions import ALL_ACTIONS
...
```python
from Softline.agent import SolanaAgentKit
from Softline.mcp.mcp_server import start_mcp_server, ALL_ACTIONS
...
## Softline MCP Server for Claude Desktop

A Model Context Protocol (MCP) server that provides on-chain tools for Claude AI, allowing it to interact with the Solana blockchain through a standardized interface. This implementation uses Softline and enables AI agents to perform blockchain operations seamlessly.


###  Claude Desktop Integration

**[Softline MCP Server](https://github.com/fastlanepython/FastlanePy-mcp)** extends Claude's capabilities with blockchain tools:
...
Datalane is licensed under the MIT License, ensuring open access and flexibility for developers. Follow updates on X at [@SoftlinePy](https://x.com/DatalanePy).
